import requests
#get PMIDs
with open('rand_pmids.txt', 'r') as randid:
    randid = randid.readlines()
    randid = ''.join(randid).replace("PMID:","")
    idlist = randid.splitlines() #by here each PMID is its own item in the list
